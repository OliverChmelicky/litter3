...
import (
	...
	custom_errors "github.com/olo/litter3/services/shared"
)
...
func (s *TrashSuite) Test_CreateTrash() {
	candidates := []struct {
		...
		{
			creator:  &User{Id: "1", FirstName: "Jano", LastName: "Motyka", Uid: "4f6f", Email: "Ja@kamo.com", CreatedAt: time.Now()},
			trash:    &Trash{Location: Point{20, 30}, Cleaned: false, Size: Size("bag"), Accessibility: Accessibility("car"), TrashType: TrashType(1)},
			updating: &Trash{Location: Point{99, 69}, Cleaned: true, Size: Size("bag"), Accessibility: Accessibility("unknown"), TrashType: TrashType(1)},
		},
	}
...
// 		collectionRequest *CreateCollectionRandomRequest
// 		randomImg         *TrashImage
// 	}{
// 		{
// 			creator:           &User{Id: "1", FirstName: "Jano", LastName: "Motyka", Uid: "Velikonoce", Email: "Ja@kamo.com", CreatedAt: time.Now()},
// 			trash:             &Trash{Location: Point{20, 30}, Cleaned: false, Size: Size("bag"), Accessibility: Accessibility("car"), TrashType: TrashType(1)},
// 			rangeRequest:      &RangeRequest{Location: Point{20, 29.99}, Radius: 5000.0},
// 			collectionRequest: &CreateCollectionRandomRequest{Weight: 32},
// 			randomImg:         &TrashImage{Url: "dasd"},
// 		},
// 	}
...
func (s *TrashSuite) Test_CreateCommentOnTrash() {
	candidates := []struct {
		...
		{
			creator:         &User{Id: "1", FirstName: "Jano", LastName: "Motyka", Uid: "8848", Email: "Ja@kamo.com", CreatedAt: time.Now()},
...
func (s *TrashSuite) Test_CreateCollectionRandom_GetCollection() {
	candidates := []struct {
		...
		{
			creator:    &User{Id: "1", FirstName: "Jano", LastName: "Motyka", Uid: "sds", Email: "Ja@kamo.com", CreatedAt: time.Now()},
			trash:      &Trash{Location: Point{20, 30}},
			request:    &CreateCollectionRandomRequest{Weight: 369.7, CleanedTrash: true},
			collection: &Collection{CleanedTrash: true, Weight: 369.7},
		},
	}
...
func (s *TrashSuite) Test_DeleteCollection() {
	candidates := []struct {
		...
		{
			creator:    &User{FirstName: "Miro", LastName: "Motyka", Uid: "sdsw", Email: "Ja@kamo.comsa"},
			trash:      &Trash{Location: Point{20, 30}},
			request:    &CreateCollectionRandomRequest{Weight: 369.7, CleanedTrash: true},
			collection: &Collection{Users: []User{}, CreatedAt: time.Now()},
			friends: []*User{
	...
	for i, cand := range candidates {
		...
		if len(candidates[i].friends) == 0 {
			...
			s.NotNil(err)
			s.EqualValues(&Collection{}, collection)
		} else {
			collection, err := s.service.TrashAccess.GetCollection(candidates[i].collection.Id)
			s.Nil(err)
			candidates[i].collection.CreatedAt = collection.CreatedAt
			candidates[i].collection.Users = []User{
				{
...
func (s *TrashSuite) Test_DeleteTrashWithComments() {
	candidates := []struct {
		creator *User
		trash   *Trash
		comment *TrashComment
	}{
		{
			creator: &User{Id: "1", FirstName: "Jano", LastName: "Motyka", Uid: "6ads", Email: "Ja@kamo.com", CreatedAt: time.Now()},
			trash:   &Trash{Location: Point{20, 30}},
			comment: &TrashComment{Message: "prva message"},
		},
	}
...
func (s *TrashSuite) Test_DeleteTrashWithCollection() {
	candidates := []struct {
		creator    *User
		trash      *Trash
		collection *CreateCollectionRandomRequest
		error      error
		code       int
	}{
		{
			creator:    &User{Id: "1", FirstName: "Jano", LastName: "Motyka", Uid: "6ads", Email: "Ja@kamo.com", CreatedAt: time.Now()},
			trash:      &Trash{Location: Point{20, 30}},
			collection: &CreateCollectionRandomRequest{Weight: 694},
